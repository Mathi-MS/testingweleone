...
import { CloudUpload, ArrowLeft, Eye, X, Video, Mail } from 'lucide-react';
...
export default function CreateNewsletter() {
    ...
    const uploadImage = async (file: File) => {
        const payload = await blogApi.uploadImage(file);
        return payload?.blobUrl || (payload as any)?.fileUrl || (payload as any)?.url || '';
    };
    ...
    const handleQuillImageUpload = useCallback(() => {
        ...
        input.onchange = async () => {
            ...
            quill.insertText(insertIndex, ' ', 'user');
            try {
                const azureUrl = await uploadImage(file);
                quill.deleteText(insertIndex, 1);
                quill.insertEmbed(insertIndex, 'image', azureUrl, 'user');
    ...
    const handleVideoFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            const payload = await blogApi.uploadVideo(file);
            const azureUrl = payload?.blobUrl || (payload as any)?.fileUrl || (payload as any)?.url || '';
            quill.deleteText(insertIndex, 1);
            quill.insertEmbed(insertIndex, 'video', azureUrl, 'user');
            quill.setSelection(insertIndex + 1, 0);
            handleContentChange(quill.root.innerHTML);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            setLoading(true);
            await blogApi.createNewsletter(formData);
            toast.success('Newsletter sent to all subscribers via email & SMS!');
            navigate('/newsletter');
        } catch {
            ...
            <div className="w-full max-w-5xl mx-auto pt-8 pb-28 px-4 sm:px-6 lg:px-8">
                ...
                <div className="flex items-center justify-between mb-8">
                    <div className="flex items-center gap-4">
                        <button type="button" onClick={() => navigate('/blog?section=newsletter')} className="p-2 hover:bg-white rounded-full transition-colors">
                            <ArrowLeft className="w-6 h-6 text-gray-600" />
                        </button>
                        <div>
                            <h1 className="text-3xl font-bold text-gray-900">Create Newsletter</h1>
                            <p className="text-gray-500 mt-1 text-sm">Content will be sent to all subscribers via email & SMS upon publishing.</p>
                        </div>
                    </div>
                    <button
                        type="button"
                ...
                {/* Subscriber notice */}
                <div className="flex items-start gap-2 text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 mb-6">
                    <Mail className="w-4 h-4 mt-0.5 shrink-0" />
                    <span>Upon publishing, this newsletter will be automatically dispatched to all subscribed users via <strong>email</strong> and <strong>SMS</strong>.</span>
                </div>
                ...
                <form onSubmit={handleSubmit} className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 space-y-8">
                    ...
                    <div className="flex items-center justify-end gap-4 pt-10">
                        <button
                            type="button"
                            onClick={() => navigate('/blog?section=newsletter')}
                    ...
                    <div className="bg-white w-full max-w-4xl rounded-2xl shadow-2xl relative">
                        ...
                        <div className="p-8 pb-16">
                            <div className="flex items-center gap-2 mb-4 px-3 py-2 bg-amber-50 border border-amber-200 rounded-lg text-xs text-amber-700">
                                <Mail className="w-3.5 h-3.5" /> Newsletter Preview — will be sent to all subscribers
                            </div>
                            {bannerPreview && (
