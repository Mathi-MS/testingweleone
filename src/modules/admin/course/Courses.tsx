...
import { getAllCourses, getMicrolearnFilterData } from "../../../features/course/courseSlice";
...
const transformFilterData = (apiData: any) => {
  ...
  if (apiData?.coursecategory) {
    filters.push({
      title: "Course Category",
      count: apiData.coursecategory.length,
      type: 'checkbox' as const,
      items: apiData.coursecategory.map((item: any) => ({
        id: item.id,
  ...
  if (apiData?.language) {
    filters.push({
      title: "Language",
      count: apiData.language.length,
      type: 'checkbox' as const,
      items: apiData.language.map((lang: string) => ({
        id: lang,
        name: lang
      }))
    });
  ...
  if (apiData?.skilllevel) {
    filters.push({
      title: "Skill Level",
      count: apiData.skilllevel.length,
      type: 'checkbox' as const,
      items: apiData.skilllevel.map((level: string) => ({
        id: level,
        name: level
      }))
    });
  ...
  if (apiData?.coursetype) {
    filters.push({
      title: "Course Type",
      count: apiData.coursetype.length,
      type: 'checkbox' as const,
      items: apiData.coursetype.map((type: string) => ({
        id: type,
        name: type
      }))
    });
...
const Courses = () => {
  ...
  const columns = [
    ...
    // { key: "promotionalContent", label: "Short FOMO Message" },
  ...
  const transformFiltersForAPI = (filters: any) => {
    return {
      ...
      categoryIds: filters["Course Category"] || [],
      languages: filters.Language || [],
      skilllevel: filters["Skill Level"] || [],
      coursetype: filters["Course Type"] || [],
...
const normalizeDuration = (duration: any) => {
  ...
  if (Array.isArray(duration.range) && duration.range.length === 2) {
    return {
      from: String(duration.range[0]),
      to: String(duration.range[1]),
    };
  }
  if (duration.from != null && duration.to != null) {
    return {
      from: String(duration.from),
      to: String(duration.to),
    };
  }
...
useEffect(() => {
  dispatch(getAllCourses({ page: 0, size: 10, searchtext: null, coursefilter: null }));
  dispatch(getMicrolearnFilterData({ page: 0, size: 4 })).then((response) => {
    console.log(response.payload, "filterdata");
    
    if (response.payload) {
      const transformedFilters = transformFilterData(response.payload);
      handleSendFilter(transformedFilters, true);
    }
  });
}, [dispatch]);
...
useEffect(() => {
  if (selectedFilters) {
    const coursefilter = removeEmptyFilters(transformFiltersForAPI(selectedFilters));
    dispatch(getAllCourses({ page: 0, size: 10, searchtext: searchText, coursefilter }));
  }
}, [selectedFilters]);
 ...
 const handleLoadMore = async () => {
    ...
    const coursefilter = selectedFilters ? removeEmptyFilters(transformFiltersForAPI(selectedFilters)) : null;
...
const handleSearch = (value: string) => {
  ...
  dispatch(getAllCourses({ page: 0, size: 10, searchtext: searchValue, coursefilter }));
};
...
useEffect(() => {
  ...
  const formattedRows = mlData.map((item: any) => ({
    ...item,
    courseTitle: (
      <Tooltip title={item.courseTitle} arrow>
      <Typography
        sx={{
      ...
      courseDescription: (
        <Tooltip title={item.courseDescription} arrow>
          <Typography
            sx={{
              fontSize: "12px",
              maxWidth: "200px",
    ...
    <>
      ...
      >
        <Box sx={{ display: "flex", alignItems: "center", gap: "10px" }}>
          ...
          <Typography sx={{ fontSize: "14px", fontWeight: "600",marginTop:"3px",color:"var(--textlight)" }}>
            {rows.length > 0 ? `(${pagination.total})` : ""} 
          </Typography>
          </Box>
        </Box>
